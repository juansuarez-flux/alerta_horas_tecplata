...
async function analyzeSprint18() {
  const sprintId = 355;
  const projectId = 'rig-website-redesign-tareas1';
  const sprintStart = new Date('2026-05-18T00:00:00Z');
  const sprintEnd = new Date('2026-05-29T23:59:59Z');

  try {
    // 1. Fetch Sprint 18 issues
    console.log('Fetching Sprint 18 issues...');
    ...
    console.log(`Found ${sprintIssues.length} issues associated with Sprint 18.`);
    ...
    // 2. Fetch all bugs created during Sprint 18 period (from 2026-05-18 to 2026-05-29)
    console.log('Fetching all issues created during Sprint 18 period...');
    // We can fetch issues in the project
    const allProjectIssuesData = await get('/issues.json', {
      project_id: projectId,
      status_id: '*',
      limit: 100,
    });
    // Wait, let's handle pagination in case there are more than 100 issues
    ...
    // Filter issues created during the sprint
    ...
    detailedSprintIssues.forEach(issue => {
      ...
      if (issue.status.id === 3) {
        // Find last transition to status Resolved (3)
        ...
        if (transition) {
          ...
          // If no journal entry, fall back to updated_on or closed_on
          resolvedDate = issue.closed_on ? new Date(issue.closed_on) : new Date(issue.updated_on);
        }
      }

      // Find start date (when status changed to In Progress (id: 2) or In Dev etc.)
      // Let's look for status change to 2 or 4, or first time it changed status from 1 (New)
      ...
      if (startTransition) {
        ...
        // Let's check any status transition
    ...
    console.log('\n--- ISSUES CREATED DURING SPRINT PERIOD (2026-05-18 to 2026-05-29) ---');
